#ifndef SPEAKER_CHANGE_ID
#include "speaker_change_id_base.h"
...
public:
    ...
    MatchedInfo checkMatchedTrack(std::shared_ptr<wespeaker::SpeakerEngine> speaker_engine, std::vector<float> current_embs)
    {
        ...
        if (matched)
        {
            ...
            if (id_tracker[matched_idx].size() < 5)
            {
    ...
    bool isEmpty() {
        return id_tracker.empty();
    }

    bool firstShow() {
        if(id_tracker.size()==1)
        {
            auto it = id_tracker.begin();
...
int main()
{
    ...
    while (params.is_running)
    {
        ...
        while (true)
        {
            int len = audio_CQ_get(&params, speaker_frame_ms, 1000);
        ...
        if (speaking_count >= speaking_threshold)
        {
            ...
            IDTrack::MatchedInfo last_match = idTrack.checkMatchedTrack(speaker_engine, current_embs);
            // idanalyzer.addID(last_match.matched_id);

            // the id of this second is first appear over past 60s
            if (last_match.is_first )
            {
                if(!idTrack.firstShow())
                {
                    idTrack.clearTracker();
                    printf("clear past id %d \n", last_match.matched_id);
                    printf("ssssssssspeaker changeeeeeeeeeeeeee \n");
                ...
                // writeDataToFile(file, overwriteOutput, last_match.matched_id, last_match.is_first, 0);
            }
            // the id of this second already matched over past 60s
            else
            {
                // double id_ratio = idanalyzer.getIDRatio(last_match.matched_id);
                printf("matched speaker id %d \n", last_match.matched_id);
                // writeDataToFile(file, overwriteOutput, last_match.matched_id, last_match.is_first, id_ratio);
            }
        }
        else
        {
