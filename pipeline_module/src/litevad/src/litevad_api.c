...
#define DEFAULT_BOS_ACTIVE_TIME  400

// 持续 700ms 检测到静音，将触发 VAD 判停
#define DEFAULT_EOS_SILENCE_TIME 300
...
#define DEFAULT_SPEECH_FRAME_TIME  30
...
short static_cast_short(float value) {
    return (short)value;
}
void convertFloatToS16LE(const float* floatData, short* s16Data, int numSamples) {
    for (int i = 0; i < numSamples; i++) {
        float scaledSample = floatData[i] * 32767.0f;
        s16Data[i] =static_cast_short(scaledSample);
    }
}
